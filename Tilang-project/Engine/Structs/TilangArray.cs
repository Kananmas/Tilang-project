...
namespace Tilang_project.Engine.Structs
{
    public class TilangArray
    {
        ...
        public static TilangVariable UseIndexer(string indexer , Processor processor) {
            TilangVariable variable = new TilangVariable();
            ...
            var parseIndexer = (string indexer) => {
                ...
                if(SyntaxAnalyzer.IsFunctionCall(splits[0])) {
                    variable = processor.ResolveFunctionCall(splits.Slice(0,1));
                }
                else {
                    variable = processor.Stack.GetFromStack(splits[0],processor);
                }
